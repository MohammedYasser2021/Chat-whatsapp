...
function App() {
    ...
    reader.onload = (event) => {
      try {
        ...
        const numbers = jsonData.map(row => {
          return row['Phone Number'] || row['رقم الهاتف'] || row['phone'] || row['رقم'];
        }).filter(Boolean);
  ...
  const sendMessages = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/send-bulk-messages`, payload);
      setResults(response.data.results);
      
      const successCount = response.data.results.success.length;
      const failedCount = response.data.results.failed.length;
      
      toast.success(`تم الإرسال بنجاح إلى ${successCount} رقم، وفشل الإرسال إلى ${failedCount} رقم`);
    } catch (error) {
